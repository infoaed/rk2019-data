...
for i in range(1, 13):
    j+=1
    #if j>10: break
    url=("https://rk2019.valimised.ee/et/voting-result/district-%d-voting-result.html") % i
    #print(url)
    succ = 1
    while succ > 0:
        try:
    ...
    dist = soup.find_all("table", {"class", "table details table-striped mb-4 d-none d-lg-block"})
    #print(dist)
    ...
    for di in dist:
        ...
        for r in rrr:
            if r.text.find("Nimekiri kokku") < 0:
                ...
                with open('rk2019-tulemused.csv', 'a') as csv_file:
